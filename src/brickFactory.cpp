...
std::vector<std::unique_ptr<Brick>> BrickFactory::createBricksUnique(
  ...
  for (int i = 0; i < 3; ++i) bricks.push_back(std::make_unique<Brick>(1));
  for (int i = 0; i < 3; ++i) bricks.push_back(std::make_unique<Brick>(2));
  for (int i = 0; i < 3; ++i) bricks.push_back(std::make_unique<Brick>(3));
  for (int i = 0; i < 3; ++i) bricks.push_back(std::make_unique<Brick>(4));
